...
export default function ProfessionalDashboard() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('users')
      .select('full_name, firm_id, firms(firm_name)')
      .eq('auth_user_id', user.id)
      .maybeSingle()
      .then(({ data }) => {
        if (data?.full_name) setFirstName(data.full_name.split(' ')[0]);
        if ((data as any)?.firms?.firm_name) setFirmName((data as any).firms.firm_name);
      });
  }, [user]);
    ...
    async function loadPacks() {
      ...
      try {
        ...
        const [{ data: allDocs }, { data: allMi }] = await Promise.all([
          ...
          supabase.from('material_information').select('property_id, water_supply, electricity_supply, broadband_speed, mobile_signal, flood_risk, coastal_erosion, coalfield_area, disputes').in('property_id', propertyIds),
        ]);
        ...
        console.error('Error loading professional packs:', err);
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-[var(--muted)] mt-1 text-sm">
              {firmName ? `${firmName} — your active transactions.` : 'Your active transactions.'}
            </p>
          </div>
          {packs.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--teal-050)] border border-[var(--border)]">
              <span className="material-symbols-outlined text-[var(--teal-600)] text-sm">work</span>
              <span className="text-xs font-bold text-[var(--teal-900)] uppercase tracking-widest">
                {packs.length} {packs.length === 1 ? 'transaction' : 'transactions'}
              </span>
            </div>
          )}
        </div>
