...
export default function ProfessionalLayout() {
  ...
  const navItems = [
    { to: '/professional/dashboard', icon: 'work',   label: 'My transactions' },
    { to: '/professional/profile',   icon: 'person', label: 'My profile' },
  ...
  const SidebarContents = () => (
    <>
      <div className="p-6">
        <Link to="/professional/dashboard" className="block group" onClick={() => setDrawerOpen(false)}>
          <div className="relative p-4 rounded-2xl border border-[var(--border)] bg-white overflow-hidden transition-colors hover:bg-[var(--teal-050)]">
            ...
            <div className="flex items-center justify-between mt-4">
              <span className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-widest">Professional</span>
              <span className="px-2 py-0.5 rounded-full bg-[var(--teal-050)] text-[var(--teal-900)] text-[9px] font-semibold uppercase tracking-wider border border-[var(--border)]">Caseload</span>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="p-4 border-t border-[var(--border)] bg-white space-y-2">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-[var(--page)] border border-[var(--border)]">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-[var(--muted)] font-medium">Professional Account</p>
          </div>
        </div>
        <button
          type="button"
  ...
  return (
    <div className="min-h-screen bg-[var(--page)] text-[var(--text)]">
      ...
      {drawerOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/40" onClick={() => setDrawerOpen(false)} />
      )}

      <aside className={`md:hidden fixed inset-y-0 left-0 z-50 w-[280px] bg-white flex flex-col transform transition-transform duration-300 ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
